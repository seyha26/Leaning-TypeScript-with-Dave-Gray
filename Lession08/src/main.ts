...
console.log(isObj("Seyha"));
console.log(isObj(true));
console.log(isObj(null));
console.log(isObj({ name: "jonh" }));
console.log(isObj(["null", "test"]));
...
const checkBooleanValue = <T>(arg: T): BoolCheck<T> => {
  ...
  if (isObj(arg) && !Object.keys(isObj).length) {
    return { value: arg, is: false };
  }
...
// console.log(checkBooleanValue({}));
// console.log(checkBooleanValue({ name: "Jonh" }));
// console.log(checkBooleanValue([]));
...
console.log(processUser({ id: 1, name: "seyha" }));
// console.log(processUser({ name: "seyha" }));
...
const myState = new StateObject<(string | number | boolean)[]>(["Seyha"]);
myState.state = ["Dev", 26, true];
